...
;;    This is mostly a 1-1 conversion. Se   e function `executable-form`.
;;
...
(defrecord LeafNode [value bindings]
  ...
  (n-to-clj [this]
    (if (not (empty? bindings))
      (let [bindings (remove (fn [[sym _]] (= sym '_)) ;; huh! is this has to do with wild-card pattern?! - Sifou 
                             bindings)]
...
(declare to-source)
;; What is the star in to-source* for? - Sifou
;; so this gives us the test, and action so we can put it 
;; a giant cond, to be used SwitchNode - Sifou 
...
;; cases here is [pattern action]
;; we call this stuff from split-matrix

;; occurences: the chosen column occurence
;; cases: the specialized matrix - need to look into that - Sifou
;; default: the default case, only one? - dito need to look into it - Sifou 
...
(defn default-specialize-matrix [p matrix]
  (let [rows (rows matrix)
        ocrs (occurrences matrix)
        focr (first ocrs)
        nrows (->> rows ;; why is col 0 passed as arg? - Sifou 
...
;;
;; In practice, the DAG compilation eliminates any wildcard patterns. ;; huh? - Sifou 
...
;; Why switch the order of the arguments? - Sifou 
...
(defn specialize-seq-pattern-matrix [rows focr]
  (->> rows
    ...
    vec))
;; It's only here that we use seq-sym, huh! - Sifou 

;; why we've used into here, instead of prepend? - Sifou 
...
;; what is this for, seriously! - Sifou 
(defn emit-patterns
...
;; so this '(quote sym) takes us to litteral-pattern?
;; to things quets transformed to quote? - Sifou
